...
from traffic.my_path import package_dir


def draw_bboxes(image, bboxes, line_thickness):
    line_thickness = (
        line_thickness
        or round(0.002 * (image.shape[0] + image.shape[1]) * 0.5) + 1
    )
    ...
    for x1, y1, x2, y2, cls_id, pos_id in bboxes:
        ...
        t_size = cv2.getTextSize(
            cls_id, 0, fontScale=line_thickness / 3, thickness=font_thickness
        )[0]
        ...
        list_pts.append(
            [check_point_x - point_radius, check_point_y - point_radius]
        )
        list_pts.append(
            [check_point_x - point_radius, check_point_y + point_radius]
        )
        ...
        ndarray_pts = np.array(list_pts, int)
...
def update(bboxes, image):
    cfg = get_config()
    cfg.merge_from_file(
        package_dir / "detect" / "deep_sort" / "configs" / "deep_sort.yaml"
    )
    deepsort = DeepSort(
