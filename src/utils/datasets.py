...
class LoadStreams:
    ...
    def __init__(
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            st = f"{i + 1}/{n}: {s}... "
            if (
                "youtube.com/" in s or "youtu.be/" in s
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any([len(x) > 8 for x in lb]):  # is segment
                    ...
                    segments = [
                        np.array(x[1:], dtype=float).reshape(-1, 2) for x in lb
                    ]  # (cls, xy1...)
                    lb = np.concatenate(
...
def dataset_stats(
    ...
    def round_labels(labels):
        ...
        return [
            [int(c), *(round(x, 4) for x in points)] for c, *points in labels
        ]
    ...
    def unzip(path):
        ...
        if str(path).endswith(".zip"):  # path is data.zip
            assert Path(
                path
            ).is_file(), f"Error unzipping {path}, file not found"
    ...
    for split in "train", "val", "test":
        ...
        for label in tqdm(dataset.labels, total=dataset.n, desc="Statistics"):
            x.append(
                np.bincount(label[:, 0].astype(int), minlength=data["nc"])
            )
