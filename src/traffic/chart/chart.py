...
def draw_line(df, output_dir):
    ...
    numeric_columns = [
        col for col in numeric_columns if col != "item_id"
    ]  # 排除item_id列
    # 设置A4纸大小（单位为英寸）
    ...
    for page in range(total_pages):
        ...
        for idx, column in enumerate(page_columns):
            ...
            for item_id, group in df.groupby("item_id"):
                ...
                if len(resampled) > 11:  # 确保有足够的点来进行滤波
                    ...
                    smooth_values = savgol_filter(
                        resampled.values, window_length=11, polyorder=3
                    )
        ...
        # 添加图例
        handles = [
            plt.Line2D([0], [0], color=color, lw=2, label=f"ID: {item_id}")
            for item_id, color in color_dict.items()
        ]
        fig.legend(
            handles=handles,
            loc="lower center",
        ...
        # 保存为PNG文件
        plt.savefig(
            f"{output_dir}/line_page_{page + 1}.png", bbox_inches="tight"
        )
        plt.close(fig)
